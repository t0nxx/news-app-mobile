...
import { getAllRepliesOfComment } from '../services/posts';
...
const CommentsReplyScreen = ({ params, navigation }) => {
    ...
    const comm = navigation.getParam('data');

    const getComments = async (page) => {
        const com = await getAllRepliesOfComment(comm.post.id, comm.id, page);
    ...
    const fetchAllDataAfterNewComment = async () => {
        const com = await getAllRepliesOfComment(comm.post.id, comm.id, 1);
        setComments([...com]);
        setIsLoading(false);
    }
    ...
    const addComment = async () => {
        ...
        try {
            ...
            console.log(payload);
            const data = await http.post('/comments/new',payload);
...
export default CommentsReplyScreen;
